    ...
    if uploaded_file:
        try:
            ...
            if auto_url_col and confidence > 0.7:
                ...
                name_col = 'name'  # Default, will be auto-generated by scraper if not exists

                # Show manual selection in expander (optional)
                with st.expander("🔧 Advanced: Change column selection"):
                    st.caption("Only needed if auto-detection is incorrect or you want to specify a custom name column")
                    col1, col2 = st.columns(2)
                    with col1:
                    ...
                    with col2:
                        # Auto-detect name column for expander
                        auto_name_col = None
                        ...
                        name_col = st.selectbox(
                            ...
                            help="Column for company names. If not in CSV, will be auto-generated from URLs."
                        )
            else:
                ...
                # Optional: Advanced name column selection
                with st.expander("🔧 Advanced: Specify name column (optional)"):
                    st.caption("Leave as 'name' to auto-generate from URLs")
                    auto_name_col = None
                    name_candidates = ['name', 'company_name', 'company', 'business_name']
                    for col in name_candidates:
                        if col in df.columns:
