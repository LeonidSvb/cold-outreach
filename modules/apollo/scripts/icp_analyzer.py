...
- Normalizes locations (abbreviations, casual forms)
- Scores companies on call center ICP fit (0, 1, 2)
- Adds reasoning for each score
- Batch processes for reliability
- Comprehensive logging with statistics

USAGE:
1. Place Apollo CSV in Downloads/
2. Run: python modules/apollo/scripts/icp_analyzer.py
3. Results saved to modules/apollo/results/
"""
...
def score_icp(
    ...
    if has_call_center_keywords(company_name, industry, headline, keywords):
        reasoning = f"Clear call center indicators found in company data. Keywords match call center operations profile."
        return 2, reasoning
    ...
    if "call" in title or "contact" in title or "customer" in title:
        if est_emp >= 10:
            reasoning = f"Title '{title}' suggests customer contact focus, but insufficient confirmation of phone-based operations."
            return 1, reasoning
...
def process_batch(df_batch: pd.DataFrame, batch_num: int) -> pd.DataFrame:
    ...
    df_batch['icp_score'] = results_df['icp_score']
    df_batch['reasoning'] = results_df['reasoning']

    logger.info(f"Batch {batch_num} complete: {len(df_batch)} rows processed")
    return df_batch


def main():
    logger.info("Starting Apollo ICP Analysis")
    ...
    try:
        ...
        logger.info(f"Loaded CSV: {len(df)} total rows")
    ...
    logger.info(f"All {num_batches} batches processed: {len(df_final)} total rows")

    score_counts = df_final['icp_score'].value_counts().to_dict()
    logger.info(f"ICP Score Distribution: Score 2: {score_counts.get(2, 0)}, Score 1: {score_counts.get(1, 0)}, Score 0: {score_counts.get(0, 0)}")
    ...
    logger.info("Apollo ICP Analysis completed successfully")
...
if __name__ == "__main__":
    try:
        ...
        logger.error(f"Script failed: {e}", error=e)
        sys.exit(1)
